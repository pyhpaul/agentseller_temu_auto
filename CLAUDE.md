...
**三层单向依赖**：`core/`（hub 公共骨架，纯净、零 automation 引用）← `features/`（依赖 core API）← `automation/`（单向依赖 core/features，feature 不反向依赖 automation）。接入契约：`registerExtension({id,panelButtons,...})`（content world，由 `registry.js` 提供）；`self.AgentSellerBg.registerHandler(prefix, fn)`（service-worker world，由 `bg-router.js` 提供）。

```
agentseller_temu/
├── core/                            # hub 公共骨架源码（纯净、零 automation 引用）
│   ├── manifest.template.json       # MV3 manifest 模板（hub 必需权限：nativeMessaging + storage）
│   ├── background/
│   │   ├── service-worker.js        # native 透传路由 + auto-reload + bg-router 数据化路由接线
│   │   ├── bg-router.js             # self.AgentSellerBg.registerHandler 前缀路由（纯逻辑，可 node --test）
│   │   ├── tab-utils.js             # self.AgentSellerBg.util.waitTabComplete（CPO + orchestrator 共用）
│   │   └── version-cmp.js           # cmpVersion（双模式纯逻辑）
│   ├── content/
│   │   ├── utils.js                 # 公共工具（挂 window.__AgentSellerUtils）
│   │   ├── ui.js                    # FAB / Panel / Hub UI 构建（挂 window.__AgentSellerUI；panel 渲染 extension.panelButtons）
│   │   ├── registry.js              # feature 注册中心 + window.AgentSeller 公开 API（含 registerExtension）
│   │   └── core.js                  # 装配入口（manifest 内排在最后一个 core 文件）
│   ├── popup/{popup.html, popup.js}
│   └── icons/icon{16,48,128}.png
├── automation/                      # 自动化系统（单向依赖 core/features；dev 装配，release 不装配）
│   ├── register.js                  # content 接入点：registerExtension({panelButtons:[打开监控]})
│   ├── bg-entry.js                  # SW 接入点：registerHandler 注册 WF_*/OPEN_MONITOR；含 orchestrator + ws-client 接线
│   ├── manifest.fragment.json       # automation 专属权限：windows + CSP(connect-src ws://localhost:*)
│   ├── contract.js                  # storage 契约常量(STORAGE_KEY/SCHEMA_VERSION/emptyBatch/normalizeSkeleton，UMD 双模式)
│   ├── orchestrator/                # 确定性编排器（engine/steps/state-machine/recovery/mutation-queue）
│   ├── brain-bridge/ws-client.js    # WS 客户端（Plan 3，SW world）
│   ├── dashboard/                   # 监控 dashboard 扩展页（ES module，独立窗口；dev-only，release 不装配）
│   │   ├── dashboard.{html,css,js}  # 壳 + 深色 tokens + ES module 装配入口
│   │   ├── state/{store,storage-source,ws-source}.js
...
│       ├── feature.json             # feature 元数据（含可选 background 字段）
│       ├── content/index.js         # feature 业务代码
│       ├── background/handler.js    # feature 的 SW 命令处理器（可选，CPO/image_search 有此文件）
...
| `background` | **可选**；相对 feature 目录的 SW 命令处理器脚本。构建时 `assemble_feature_backgrounds` 将其 importScripts 注入 SW 末尾；脚本内部须调 `self.AgentSellerBg.registerHandler(prefix, fn)` 注册命令。无此字段的 feature 零改动（向后兼容）。 |
...
window.AgentSeller = {
  ...
  registerExtension({ id, panelButtons }),  // automation 接入点（content world）；feature 不调此 API
...
**SW world 注册表**（`core/background/bg-router.js` + `core/background/service-worker.js`）：

```js
self.AgentSellerBg = {
  registerHandler(prefix, fn),          // feature/automation bg 脚本调此注册命令处理器（前缀路由）
  util: {
    waitTabComplete(tabId, timeout),    // 等 tab 加载完成；CPO + orchestrator 共用（core/background/tab-utils.js）
  },
};
```

- `automation/bg-entry.js` 调 `registerHandler('WF_', ...)` + `registerHandler('OPEN_MONITOR', ...)` 注册编排器和监控窗口。
- `features/create_purchase_order/background/handler.js` 调 `registerHandler('CPO_', ...)` 注册 CPO 命令。
- `features/image_search_1688/background/handler.js` 调 `registerHandler('IMG_SEARCH_', ...)` 注册截图命令。
- 以上脚本均经 build 的 `assemble_feature_backgrounds` importScripts 注入 SW 末尾，运行时与 SW 共享同一 global scope。
...
| `OPEN_FOLDER` | 用资源管理器打开文件夹（入参 path；Windows `os.startfile`，非 Windows/路径不存在返回 error，调用方 silent 兜底） |
...
### service-worker.js 的职责

`core/background/service-worker.js` 职责 = **native 透传 + auto-reload + bg-router 数据化路由注册表**。SW 不再硬编码任何 feature/automation 专属分支；feature 和 automation 的 bg 逻辑经 build 的 `assemble_feature_backgrounds` importScripts 注入 SW 末尾，并在自己的脚本里调 `self.AgentSellerBg.registerHandler(prefix, fn)` 注册命令处理器。

当前已归位的 feature bg：
- `features/create_purchase_order/background/handler.js`：CPO 跨 tab 编排（`cpoRun/cpoRun2`），注册前缀 `CPO_`
- `features/image_search_1688/background/handler.js`：截图跨 tab 编排，注册前缀 `IMG_SEARCH_`

**何时把逻辑放 feature background**：feature 需要「跨多个 tab/域名按序操作」（开/关 tab、等加载、发命令、收数据、再下一步）时——只有 service worker 跨 tab 存活。此时模式是：
...
> **⚠️ CPO 不 IIFE 封装**：`cpoRun`/`cpoRun2` 故意保留在 SW 全局 scope（不包 IIFE），因为 `automation/bg-entry.js` 的 orchestrator adapter 直调这两个函数（受控的同 scope 共享，非模块反向依赖）。两者都被 build 装配进同一 SW、共享 global，`await` 可靠等终态。
...
> 来源：auto_gen_label v1.1.1 连环 bug 复盘。表象是"选 SKC 超时"，真根因是"按 SKC ID 查询时抓**全页第一个 SPU**、零校验"，导致**全程操作错误商品**（填错合规 + 标签图传错商品）+ 历史数据污染。"操作了错误对象"比"流程失败"严重得多——失败会停下，错配会**静默污染下游**且事后极难定位。

**1. 查询取值禁止抓"第一个匹配"。** 按 key 查询后取值，禁止 `querySelectorAll(...)[0]` / `body.innerText.match()` 抓第一个；用唯一性或字段交叉校验确认取到的是目标。精确 key（如 SKC）结果应唯一 → 轮询等"含目标字段的结果行**恰好 1 行**"再取，不唯一/超时报错中止（参考 `auto_gen_label` 的 `extractSpuFromUniqueResult`）。**rowspan 分组场景**（一商品跨多 tr）下"恰好 1 行"应理解为"含目标字段的**分组/首行恰好 1 个**"——按 tr 数判唯一会因多 SKU 行误判不唯一。**Why**：结果未刷新 / 有默认数据 / 多结果 / 虚拟滚动时，"第一个"是错误对象；错误从源头进入，下游所有校验都**自洽地错**、查不出来。
...
**automation 发版隔离（目录级装配）**：dashboard / overlay / orchestrator / ws-client / 监控按钮均在 `automation/`，release 走 `build_all(with_automation=False)` → 不装配 `automation/` → 产物天然无 dashboard/overlay/windows/CSP，无需任何 strip 补丁。`package_all.py` 调 `build_all(with_automation=False)`（三个旧 strip 函数已删）。Release manifest 比 main dev 产物更干净（去除了 automation 专属的 `windows` 权限和 CSP，以及 overlay content scripts）。

dashboard 未来「转正」纳入发版：`package_all.py` 改回 `build_all(with_automation=True)` 即可（无其他改动）。
