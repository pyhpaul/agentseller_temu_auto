...
| bg→tab 命令 | `CPO_P2_WAIT_SEARCH` | `{poNo}` | 待到货页用采购单号搜索定位商品行（停在申请付款前；v1.2.2 起新品+复购统一用 PO 号搜，不再用 skuNo） |
...
| 「已存在」弹窗 | `.ant-modal / .ant-modal-confirm / .modal`（精确结构待 dump，当前靠关键词+可见性鲁棒检测），文案含「采购单号：PO...已存在,不能重复添加」 | 关键词 `/已存在\|不能重复添加\|已完成/` 检测 → 已入库分流 |
...
14. **跨 tab 编排：新 tab 在原 tab 旁开 + error 切回原 tab（体验）**
    ...
    - **fresh tab handler 必须以 `waitForEl` 开头**：`tab.status==='complete'`/`self.AgentSellerBg.util.waitTabComplete` ≠ Vue 组件已渲染，新开/刚导航的 tab 直接 querySelector 会概率性落空（ADD_FETCH/EDIT_FILL/EDIT_SAVE/WAIT_SEARCH 均已补）。
