...
> 粒度说明：采集结果是**去重后 SKC 粒度**；页面「共有 N 条」是表格行数（**SKU 粒度**，一个 SKC 含多个 SKU 行）。
> 两者数值不一致是设计预期，不是 bug，故第 6 条只提示不硬校验。
...
- rowspan 分组：每 SKC 组 = 首行（含商品信息格，四字段都在这）+ N 个 SKU 行 + 1 个「合计」行（无商品信息格，扫描时自然跳过）。代码只提取每组首行数据（四字段都在首行），SKU 行/合计行不单独取值。
- 标准分页器（非虚拟滚动）：`beast-core-pagination`；末页 next 含 `PGT_disabled`。
- **单页（数据不足一页）时分页器渲染但 next 按钮不渲染**（端到端实测）——next 缺失不能一律
  当读取故障；用页码项数量区分：`PGT_pagerItem` ≥2 个才是真故障（多页却找不到 next）。
...
3. 逐页 `collectPageRows`（每 SKC 组只取首行代表）→ `Map<SKC,row>` 去重 → `clickNextAndWait` 翻页。
4. **翻页后必须等内容签名（首组SKC|末组SKC|组数）变化**，spin 遮罩消失不够（auto_ship #47 同款坑）。
   **签名禁止含激活页码**：点 next 后页码立即变、表格数据 4-5s 后才到（端到端实测），
   ...
   `clickNextAndWait` 每次点击等 15s 内容变化，没变就重点（≤3 次）；变化后校验激活页码 == 期望页，
   跳页（双点击都生效）则中止防漏采。
