...
- **状态**：已实施（`feature/hub-automation-layering`，阶段 0/1/2 完成）
- **日期**：2026-06-13
...
```js
// automation/register.js（content world 接入点）
window.AgentSeller.registerExtension({
  id: 'automation',
  panelButtons: [{ id: 'open-monitor', icon: '📊', label: '打开监控', onClick }],
  // overlays 由 overlay.js 自挂载（搬迁阶段保持自驱逻辑，不经 registerExtension 统一管理）
});
```

**World 边界澄清（实现关键）**：`registerExtension` 属于 **content world**，只能注册 `panelButtons`（UI 按钮）；它**不包含** `bgHandlers` 字段——SW world 的命令处理器经 **`self.AgentSellerBg.registerHandler(prefix, fn)`** 注册，这是两个机制、两个隔离 world。监控按钮 onClick 在 content world 发 `OPEN_MONITOR` 消息 → SW 侧 `registerHandler('OPEN_MONITOR', ...)` 处理。

core 侧职责（**只认契约，不认 automation**）：

- `registry.js`：维护 `extensions` 注册表；暴露给 `ui.js` 读取（`collectPanelButtons`）。
- `ui.js`：遍历所有 extension 的 `panelButtons` 渲染进 Panel（取代当前硬编码的 `__monitorBtn`）。
- `core/background/bg-router.js`：`makeBgRouter()` 前缀路由，挂 `self.AgentSellerBg.registerHandler`；feature/automation bg 脚本各自调此 API 注册命令。
...
| `registry.js` 的 `openMonitor` | **全删**。降为 automation `register.js` 经 `registerExtension` 注册的 panelButton |
| `overlay*.js` 散落 isDev 判断 | **全删**。overlay 整体在 automation/，dev-only 由目录决定 |

> **"release 产物比 main dev 更干净"**：release 移除了 main dev 残留的 overlay content scripts（本就是 automation 专属，泄漏了 dev 产物但不影响功能）、windows 权限、CSP——比旧方案"release 与 main 零差异"更正确，是结构保证而非补丁。
...
**实施后（决策 A：保持 adapter 直调全局 cpoRun，不改为命令入口+轮询）**：

```
Panel 手动按钮 ──CPO_START──▶ CPO bg handler（registerHandler 'CPO_'）
                                    └ cpoRun/cpoRun2 写 cpo_state（自治）
orch adapter ──────────────────────▶ await cpoRun/cpoRun2（同 SW global scope 直调）
                                    └ 读 cpo_state 映射进 as_workflow_state.step.result
```

- **决策 A 理由**：automation 与 CPO 都被 build 装配进同一 SW global scope，adapter 直调是受控的同 scope 函数共享（非模块反向依赖）；`await` 可靠等终态，优于「命令入口+轮询 storage」（后者存在 SW 5 min await 限制风险）。
- `cpoRun`/`cpoRun2` **故意不包 IIFE**，保持在 SW global 供 adapter 直调。
- **storage 仍两 key**（决策 5）：`cpo_state`（CPO 自治）+ `as_workflow_state`（automation 编排）。CPO 对 automation 零反向依赖。
- 映射（读 cpo_state → 填 step.result）是 **automation adapter 的单向职责**，在 `automation/bg-entry.js` 的 `orchAdapterCreateSku`/`orchAdapterCreatePo` 内。

### 7.2 overlay STORAGE_KEY

overlay 随 contract 同搬入 `automation/`，直接 `import { STORAGE_KEY } from '../contract.js'`，删除硬编码常量。

### 7.3 错误结构统一（adapter 翻译层，Plan 3 已实现）

CPO bg handler 保留私有错误结构（`{status, label}`），**不返回 automation 的 StepError**（否则反向依赖 contract，违背决策 5）。统一在 **automation adapter 翻译层**完成：`orchAdapterCreateSku`/`orchAdapterCreatePo` 读 `cpo_state.phase1`/`phase2` 后，自行构造 `StepError{category, code, message, recoverable}`（category 按 label 文案归类 read/validate/business）。CPO 的 `content/index.js` 消息协议不动。

> Plan 3 阶段已实现：adapter 内已有错误翻译（见 `automation/bg-entry.js` 的 `orchAdapterCreateSku`/`orchAdapterCreatePo`）。
