...
| **dashboard 操作中心化(2026-06-15)** | ①选品步记录 `sourceUrl` 锚点(product 新增字段 + pickProduct 白名单，解 bg-entry「数据流死结」起点，后续承接 url 驱动采集) ②**HITL 操作全搬 dashboard**:hitl-queue 按类型(回填输入框/复核 concerns/纯确认)可交互渲染 + error 重试/转人工 + `onHitlAction` 接通 `WF_*` 回路(复用 overlay-view 纯逻辑) ③**overlay 降级只读进度**(去操作按钮) ④**workflow 生命周期**:中止条件化(仅 running/paused/error)+ 删除(`WF_DELETE`)+ 重启三档(`WF_RESTART`:重头/当前步/任意步,重头清 product 保留 label) | ✅ 合入 main(PR #70)· 子 agent review APPROVE(0 严重 / 3 次要不阻塞)· node 128 / pytest 90 · **stub 端到端验证**(13 步跑通 + 三判断点回流 dashboard + 选品 url 存 product + 删除/重启/中止条件化均验)· 真模型 + real adapter e2e 仍人工 gated · plan `docs/superpowers/plans/2026-06-15-dashboard-hitl-operation-center.md` |
