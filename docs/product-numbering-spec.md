...
- **唯一性**：SKC 级（同一 SKC 的所有 SKU 行该值**相同**）
- **来源**：Temu 条码管理页表格「SKC货号」**独立列**（与 SKU货号 是两列，不要混）
- **例**：`CLI319`、`RAC-020`
- **特点**：
  - 包含商品型号/分类信息（可人类识别）
  - ⚠️ **同一 SKC 多个 SKU 行的 SKC货号相同** —— 绝不能用它当行/SKU 的区分键

### SKU货号（SKC货号 + 属性）
- **含义**：具体商品变体的完整编号 = SKC货号 + 颜色/尺寸等属性
- **唯一性**：SKU（变体）级（一个 SKC 可对应多个 SKU 货号，各行不同）
- **来源**：Temu 条码管理页表格「SKU货号」**独立列（与 SKC货号 分开的另一列）**。
  ⚠️ 不是从 SKC货号 split 出来的，是独立列，直接读取
- **例**：`CLI319-White-2pcs`、`RAC-020-Black`、`RAC-020-White`
- **特点**：
  - 空值在表格里显示为占位符 `-`（读取时归一为空）
  - 是标签文件名 + 条码标签序列号的取值来源
